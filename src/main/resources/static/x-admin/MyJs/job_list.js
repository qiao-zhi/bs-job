...
function showUsersTable(pageInfo){
    ...
    for(var i=0,length_1 = users.length;i<length_1;i++){
        ...
        var tr = "<tr>"
            +'<td>'+index+'</td>'
            +'<td>'+replaceNull(users[i].jobname)+'</td>'
            +'<td>'+replaceNull(users[i].description)+'</td>'
            +'<td>'+replaceNull(users[i].needNumber)+'</td>'
            +'<td>'+replaceNull(users[i].createtime)+'</td>'
            +'<td>'+replaceNull(users[i].username)+'</td>'
            +'<td>'+replaceNull(users[i].fullname)+'</td>'
            +'<td>'+replaceNull(users[i].contactUserFullname)+'</td>'
            +'<td>'+replaceNull(users[i].contactUserPhone)+'</td>';

        if(users[i].receiverNum != null && '' != users[i].receiverNum){
            tr += '<td><a href="javascrip:void(0)" title="查看详情" onclick="queryUserjobs(\''+users[i].id+'\')">'+replaceNull(users[i].receiverNum)+'</a></td>'
        }else{
            tr += '<td>'+replaceNull(users[i].receiverNum)+'</td>'
        }

        tr+='<td><a href=javascript:void(0) title="修改" onclick="updateUser(\''+users[i].id+'\')"><i class="layui-icon">&#xe642;</i></a>'
            +'<a href=javascript:void(0) title="删除" onclick="deleteUser(\''+users[i].id+'\')"><i class="layui-icon">&#xe640;</i></a>'
            +'<a href="/message/getMessagedetail/'+users[i].id+'" target="_blank" title="查看详情"><i class="layui-icon"></i></a>';
            +'</td></tr>';
...
function queryUserjobs(id){
    var url = '/userJob/getUserjobs.html?jobId='+id;
    x_admin_show('招募信息',url,1000,400);
}